...
let optionDataConverter = function () {
  ...
  let optionDetailContentList = document.getElementById("optionDetailContentList");
  let optionDetail = [];
  [].slice.call(optionDetailContentList.children).forEach(function (p) {
    optionDetail.push({
      ...
      optionPriceDifference: p.querySelector(".optionDetailPriceDifferenceInput").value
...
let productDataConverter = function () {
  ...
  if (groupId == "") {
    toastr.warning("Grup Ekleyiniz.", "Dikkat!");
    throw new Error("Grup Ekleyiniz.");
  }
